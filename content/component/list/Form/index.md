...
| 类型 | 标题（A） | 标签（B） |输入域（C） |提示信息（D） |
| :--: | :-------: | :-------: | :-------: | :-------: |
| 常规 |     可选     |     √     | √     | 可选     |



A 标题：帮助对信息进行分组，方便用户填写；

B 标签：标签文本主要是解释输入项的含义，一般不宜太长，需要简明扼要，快速让用户理解；另外，还可以通过标签告知用户哪些是必填项；

C 输入域：可交互输入的区域，是构成表单的核心内容，用来收集用户操作的信息。作为容器主要放置：单选 `Radio`、 数字输入框 `NumberInput`、 输入框 `Input`等组件；

D 提示信息：包含帮助信息、占位符和反馈信息提示 。
