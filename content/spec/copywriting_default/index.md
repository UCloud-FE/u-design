...
---
description: 基于常见的平台文案，结合体验设计思路，提供文案设计的通用原则"
---
...
**2.合理选择“通知”、“提示/说明”、“建议”、“告警”、“报错”的方式表达信息**
...
- ***「案例」操作按钮文案***
![](https://www-s.ucloud.cn/2022/11/07f2262e1b6ccccdb8165aff7d40df62_1668157349383.png)
> 配图：主机、数据库、网络等产品列表页一级按钮都是“创建XX”｜详情页监控卡片上的操作也是一致的
...
- ***陈述句（纯文本说明）：建议使用常规主谓宾语序对事物进行清晰表述。

> “SSD机型适用于对数据库性能要求较高的业务场景。”

- ***祈使句（告警/报错）：建议省略不必要的解释说明，优先提供纠错方案及引导。***

> “请输入管理员密码”

- ***陈述+引导句式（操作反馈）：简短阐述结果并给到必要的下一步操作引导，如有跳转建议添加。***

> “支付失败。你的账户余额不足，请 [充值]() 重新购买。”

- ***优先短句及短语：简单逻辑下，关注效率。短句或短语相比于语法准确的长句更易突出重点以便读者理解。***

> 推荐：切换至 [VNC登录]()
>
> 不推荐：如需使用VNC方式访问主机，请[点击这里]()
...
- **正确使用标点符号**：除了常用的标点符号外，对于冒号、引号、破折号等的使用也应该遵从国家标准《标点符号用法（**GB/T 15834-2011**）》。
...
>   - **购买、订单信息、账户流水**的场景下展示扣除/退还金额，变更主体是用户账户：**正数为消费，负数为退费**
>   - **删除资源、变更配置**的场景下展示补交/退还金额，变更主体是原消费记录：**正数为退费，负数为应补差价**

![金额显示的示意图，需要对齐以上几点]()----订单列表，进出账户金额展示
...
> 中文语境中引用英文词汇辅助说明时，英文前后仍需使用中文标点符号。

- ***避免使用缩写，正确使用大小写***

> 混排时若涉及一些专用词汇，请使用全称。例如，使用“Javascript”而非“js”。下

- ***中文语境内的英文前后需增加空格（英文与中文标点间无需空格）***

> 正确：Steve Jobs 那句话是怎么说的？「Stay hungry，stay foolish.」
>
> 错误：Steve Jobs 那句话是怎么说的？「 Stay hungry ，stay foolish .」
...
一般为列表空提示、输入组件placeholder、缺省模版文案等。根据不同信息容器，展示于列表、输入组件和独占页面中。一般出现在数据生成之前，用于告知用户当前无数据。根据产品需要可以增加产品介绍、文档链接。[了解详情>>]()

- ### 操作反馈

一般分为前端反馈、后端反馈。等展示于操作触发后，在告知用户操作执行的同时将操作结果反馈给用户。操作执行分前端执行和后端执行，后端执行分同步执行和异步执行。根据不同的执行方式，反馈的时间点、展现形式及传达信息各有不同，需要根据具体情况具体分析。[了解详情>>]()
